...
const ItemsList = ({ items }) => {
  ...
  return (
    <>
      ...
      {Object.entries(groupedItems)?.map(([date, itemList]) => (
        <div key={date}>
          <h3>Date: {date}</h3>
          ...
          <table className="w-full border border-black my-3">
            <thead>
              <tr>
                ...
                <th className="border border-black bg-slate-400">Item Name</th>
            ...
            <tbody>
              {itemList.map(({ name, amount, quantity, _id }) => (
                <tr key={_id}>
                  ...
                  <td className="border border-black text-center">{amount}</td>
            ...
            <tfoot className="bg-slate-200">
              ...
              <td className="text-center">
                <dt>{itemList.reduce((sum, item) => sum + item.amount, 0)}</dt>
              </td>
              <td />
              <td />
            </tfoot>
